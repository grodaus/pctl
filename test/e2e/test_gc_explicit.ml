...
let () =
  ...
  let _ = Harness.down ~scratch:a in
  let id_a_s = Schema.Project_id.to_string (Harness.project_id a) in
  (* Disable opportunistic sweep from affecting this path; we want gc --yes
   * to do the removal explicitly. Keep the project_dir on disk so class is
   * Orphan (not Unknown). *)
  ...
  Harness.assert_not_contains ~label:"gc --yes cleared project a" list_out
    id_a_s;
