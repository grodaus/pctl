...
let () =
  ...
  | Some why ->
      Printf.printf "SKIP: test_results — %s\n" why;
      exit 0
  | None ->
      ...
      (Harness.with_scratch
         ~services:
           [
             ...
             simple "slow"
               (Printf.sprintf
                  "%s -c 'sleep 0.5; exec %s -c \"while true; do sleep \
                   3600; done\"'"
                  bash bash);
           ]
       @@ fun scratch ->
       ...
       if rc = 0 then
         Alcotest.failf "expected results to exit non-zero for mixed \
                         outcome, got rc=%d stdout=%s"
           rc out;
       if elapsed > 8.0 then
         Alcotest.failf "results took too long (%.2fs > 8s)" elapsed;
       if not (contains out "ok") then
         Alcotest.failf "stdout should contain 'ok': %s" out;
       if not (contains out "fail") then
         Alcotest.failf "stdout should contain 'fail': %s" out;
       if not (contains out "slow") then
         Alcotest.failf "stdout should contain 'slow': %s" out;
       if not (contains out "failed") then
         Alcotest.failf "stdout should mention 'failed': %s" out;
       ...
       if List.length items <> 3 then
         Alcotest.failf "expected 3 records, got %d: %s" (List.length items)
           out_j;
       List.iter
         (fun item ->
      ...
      let rc, out = Harness.results ~scratch ~timeout:10 () in
      if rc <> 0 then
        Alcotest.failf "expected happy-path results exit 0, got rc=%d \
                        stdout=%s"
          rc out;
      if not (contains out "ok") then
        Alcotest.failf "stdout should contain 'ok': %s" out;
      if not (contains out "slow") then
        Alcotest.failf "stdout should contain 'slow': %s" out;
      if contains out "failed" then
        Alcotest.failf "happy path should not mention 'failed': %s" out;
