...
let () =
  Harness.skip_or_run ~name:"test_up_no_wait" @@ fun () ->
  let web =
    {
      Harness.name = "web";
      probe =
  ...
  Harness.check_rc_zero ~label:"plain up" (Harness.up ~scratch);
  let elapsed = Unix.gettimeofday () -. t0 in
  if elapsed > 3.0 then
    Alcotest.failf
      "plain up was slow, maybe waiting by accident (%.2fs > 3s)" elapsed;
